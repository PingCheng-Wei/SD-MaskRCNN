...
class SDMaskRCNNModel(object):
    ...
    def detect_dataset(
        ...
        for image_id in tqdm(dataset.image_ids):
            ...
            ########################################################################
            ''' only uncomment it when you also want to generate the resized
                ground truth segmantic mask in directory 'modal_segmasks_processed'
            '''
            # Load image and ground truth data and resize for net
            image, _, _, _, gt_mask = modellib.load_image_gt(
                dataset, self.mask_config, image_id, use_mini_mask=False
            )
            ''' Otherwise we just use dataset.load_image()'''
            # # load infernce image
